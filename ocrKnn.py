...
def updateKnn(knn, train, trainLabel, newData=np.array(None), newDataLabel=np.array(None)):
    if (newData!=None).all() and (newDataLabel!=None).all():
...
while True:
    ret, frame = cap.read()
    frame = frame[:,:426]
    rois, edges = findRoi(frame, 30)
    digits = []
    for r in rois:
        x, y, w, h = r
        digit, th = findDigit(knn, edges[y:y+h,x:x+w], 30)
    ...
    if key == ord(' '):
        ...
        try:
            ...
            continue
        knn, train, trainLabel = updateKnn(knn, train, trainLabel, output, np.array(numbers))
